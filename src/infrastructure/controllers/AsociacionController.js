...
const AsociacionService = require('../../application/services/AsociacionService')

class AsociacionController {

  async create(req, res) {

    await body('name').notEmpty().withMessage('El nombre es obligatorio').matches(/^[a-zA-Z\s]+$/).withMessage('El nombre solo puede contener letras').run(req)
  ...
  async update(req, res) {
    ...
    await body('name').matches(/^[a-zA-Z\s]+$/).withMessage('El nombre solo puede contener letras').run(req)
